...
function AddHome() {
	...
	useEffect(() => {
		const fetchHoas = async () => {
			await axios.get(`hoas`).then((response) => {
	...
	async function Submit(e) {
		...
		try {
			await axios
				...
				.then((response) => {
					alert(
						'Request Submitted! Wait for admin to approve your request.'
					);
					navigate('/homes');
				});
		} catch (err) {
					...
					<>
						...
						>
							<TextField
								...
								variant="filled"
								onChange={(e) =>
									updateForm({ houseName: e.target.value })
								}
							...
							<div className="FormWrapper__2">
								...
								<TextField
									...
									variant="filled"
									onChange={(e) =>
										updateForm({ phase: e.target.value })
									}
			...
			<div id="SectionHolder">
				<section className="Section">
					...
					<div className="SectionStepper">
						...
						<Button
							variant="text"
							className={stepper === 2 ? 'active' : ''}
							onClick={() => setStepper(2)}
						>
							Join Homeowners Association
						</Button>

...
function AuthProvider({ children }) {
    ...
    const isAdmin = (id) => {
        const data = (!localStorage.getItem('adminOf')) ? [] : localStorage.getItem('adminOf')
        if (data == id){
            return true
        }
        return false
    ...
    const isGuard = (id) => {
        const data = (!localStorage.getItem('guardOf')) ? [] : localStorage.getItem('guardOf')
        if (data == id){
            return true
        }
        return false
    ...
    const isHomeowner = (id) => {
        const data = (!localStorage.getItem('homeownerOf')) ? [] : localStorage.getItem('homeownerOf').split(",")
        if (data.includes(id)){
            return true
        }
        return false
    ...
    const isResident = (id) => {
        const data = (!localStorage.getItem('residentOf')) ? [] : localStorage.getItem('residentOf').split(",")
        if (data.includes(id)){
            return true
        }
        return false
    ...
    const isRole = (id) => {
        const data = (!localStorage.getItem('roles')) ? [] : localStorage.getItem('roles').split(",")
        if (data.includes(id)){
            return true
        }
        return false
